...
export async function checkAndUpdateDelayedStatus(orderId: string): Promise<boolean> {
  try {
    ...
    if (isDelayed && order.status !== 'DELAYED') {
      ...
      console.log(`[Delay Service] Order ${orderId} marked as DELAYED (${elapsedMinutes} min > ${order.estimatedDuration} min)`);
      return true;
    }
    ...
    if (!isDelayed && order.status === 'DELAYED') {
      ...
      console.log(`[Delay Service] Order ${orderId} reverted from DELAYED to OUT_FOR_DELIVERY`);
      return false;
    }
